...
//!
//! Product overlays that land from parallel tracks (`notes`, `chrome_ui`,
//! `workspace_ui`, `transfer_ui`) are added to this export list when their
//! modules merge — keep this file the single host-facing surface.
//!
//! Rendering (`renderer`, `text`, `app`, rain, wgpu) stays **binary-only** so
//! library consumers can plug their own present loop or spawn the binary.
//!
