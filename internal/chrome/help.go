...
func helpBody(windowCols int) string {
	...
	body = append(body, helpRow(inner, KeyCtrlShift("M"), "Notes (opens last note; saved)"))
	body = append(body, helpRow(inner, "Esc", "Notes: editor→list · list→close"))
	body = append(body, helpRow(inner, "↑↓ / Enter / n / d", "Notes list: move · open · new · delete"))
	body = append(body, helpRow(inner, "F2 / click title", "Notes: rename in editor"))
	body = append(body, helpRow(inner, "Esc", "Dismiss other overlays"))
	body = append(body, "")
