...
func helpBody(windowCols int) string {
	...
	body = append(body, helpRow(inner, KeyCtrl("/"), "Help (this window)"))
	body = append(body, helpRow(inner, "Esc", "Dismiss overlay"))
	body = append(body, "")
	body = append(body, helpSection(inner, "Notes"))
	body = append(body, helpRow(inner, KeyCtrlShift("M"), "Toggle notes (opens last note)"))
	body = append(body, helpRow(inner, "Esc", "Editor → list · list → close"))
	body = append(body, helpRow(inner, "↑↓ / Enter", "List: move · open note"))
	body = append(body, helpRow(inner, "n / d", "List: new · delete"))
	body = append(body, helpRow(inner, "F2 / click title", "Editor: rename note"))
	body = append(body, helpRow(inner, KeyCtrl("A")+"/"+KeyCtrl("C")+"/"+KeyCtrl("X")+"/"+KeyCtrl("V"), "Editor: select / copy / cut / paste"))
	body = append(body, "")
