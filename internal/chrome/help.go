...
func helpBody(windowCols int) string {
	...
	body = append(body, helpRow(inner, KeyCtrlShift("M"), "Notes (saved bank)"))
	body = append(body, helpRow(inner, KeyCtrl("N")+" / PgUp·PgDn", "Notes: new / switch (with Ctrl)"))
	body = append(body, helpRow(inner, KeyCtrl("A")+" / Tab", "Notes: select all / tab char"))
