...
func helpBody(windowCols int) string {
	...
	body = append(body, helpRow(inner, "Ctrl+←→", "Word jump (bar · notes · Grok)"))
	...
	body = append(body, helpRow(inner, KeyCtrlShift("M"), "Toggle notes (context keys appear with UI)"))
	body = append(body, helpRow(inner, "Esc", "Editor → list · list → close"))
	body = append(body, "")
	body = append(body, helpSection(inner, "Zoom"))
	body = append(body, helpRow(inner, "⌘+/⌘- · Ctrl++/Ctrl+-", "Larger / smaller UI"))
	body = append(body, helpRow(inner, "⌘0 · Ctrl+0", "Reset zoom (default size)"))
	body = append(body, "")
